...
export const Route = createFileRoute("/layanan/$slug")({
  ...
  head: ({ loaderData, params }) => {
    ...
    const desc = s ? `${s.description} Layanan ${s.title.toLowerCase()} area Karawang, Cikampek, Purwakarta, Bekasi.` : "";
    return {
      meta: [
        { title },
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-4 py-32 text-center">
        ...
        <Link to="/layanan" className="mt-4 inline-block text-primary">← Kembali ke daftar layanan</Link>
      </div>
    </SiteLayout>
  ),
});
  ...
  return (
    <SiteLayout>
      <article className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20 lg:px-8">
        <Link to="/layanan" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2 lg:items-start">
          <div className="overflow-hidden rounded-2xl border border-white/10 shadow-elegant">
            <img src={service.image} alt={`${service.title} oleh Bara Baja Las Karawang`} className="aspect-[4/3] w-full object-cover" />
          </div>
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <GlowButton
                as="a"
                href={waLink(`Halo ${SITE.name}, saya tertarik dengan layanan ${service.title}. Mohon info lebih lanjut.`)}
                target="_blank"
